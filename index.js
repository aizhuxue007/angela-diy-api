...
app.put('/jokes/:id', (req, res) => {
  let foundJoke = jokes.filter(joke => joke.id === req.params.id);
  const updatedJoke = {
    id: req.params.id,
...
app.patch('/jokes/:id', (req, res) => {
  ...
  keys.forEach(key => {
    const jokeKey = `jokeT${key.slice(1)}`;
    jokes[searchIndex][jokeKey] = req.body[key];
  })
...
app.delete('/jokes/:id', (req, res) => {
  const jokeIndex = jokes.findIndex(joke => joke.id === Number(req.params.id))
  delete jokes[jokeIndex];
  res.status(200).send(`ok ${jokes[jokeIndex]}`)
});
